...
class GameConsumer2(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.room_group_name not in self.rooms.keys():
            GameConsumer2.rooms[self.room_group_name] = {"players": {"player1": self}}
            GameConsumer2.rooms[self.room_group_name]["key_states"] = {}
            print('primer usuario' + str(GameConsumer2.rooms[self.room_group_name]))
            
        else:
            print('segundo:' + str(GameConsumer2.rooms[self.room_group_name]))
            GameConsumer2.rooms[self.room_group_name]['players']['player2'] = self
            GameConsumer2.rooms[self.room_group_name]["key_states"] = {}
    ...
    async def receive(self, text_data):
        	self.rooms[self.room_group_name]['key_states'] = json.loads(text_data)
    ...
    async def waiting_players(self, ball):
        print(self.rooms[self.room_group_name]["key_states"])
        while True:
            if self.rooms[self.room_group_name]["key_states"]:
                if 'Digit1' in self.rooms[self.room_group_name]["key_states"]:
                    print("Player 1")
                    self.players = 1                # 1 -> Player vs CPU
                    return copy.copy(ball)
                elif 'Digit2' in self.rooms[self.room_group_name]["key_states"]:   # 2 -> PvP
                    self.players = 2
    ...
    async def waiting_countdown(self):
        while True:
            if self.rooms[self.room_group_name]["key_states"]:
                if 'F15' in self.rooms[self.room_group_name]["key_states"]:
                    break
            await asyncio.sleep(0.1)
    ...
    async def playGame(self):
        ...
        score = Score()
        await self.rooms[self.room_group_name]['players']['player1'].send_gameboard(ball, left_paddle, right_paddle, score)
        await self.rooms[self.room_group_name]['players']['player2'].send_gameboard(ball, left_paddle, right_paddle, score)
        ...
        while True:                                              ### GAME LOOP ###
            ...
            await self.rooms[self.room_group_name]['players']['player1'].send_gameboard(ball, left_paddle, right_paddle, score)
            await self.rooms[self.room_group_name]['players']['player2'].send_gameboard(ball, left_paddle, right_paddle, score)
            handle_paddle_movement(self.rooms[self.room_group_name]["key_states"], left_paddle, right_paddle, self.players)
            if self.players == 1:
            ...
            while (self.rooms[self.room_group_name]["key_states"].get('F14')):
                await self.rooms[self.room_group_name]['players']['player1'].send_gameboard(ball, left_paddle, right_paddle, score)
                await self.rooms[self.room_group_name]['players']['player2'].send_gameboard(ball, left_paddle, right_paddle, score)
                await asyncio.sleep(0.1)
